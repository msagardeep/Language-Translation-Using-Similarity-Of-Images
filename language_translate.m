function [closestMatches] = language_translate(databaseDirectory)
...
test_sift_hist = hist_hsv_test(databaseDirectory);
...
result_cat_0 = {};
for k=1:100
    result_cat_0{k} = test_objs{I(k)};
end
% create a webpage showing the results
fid = fopen('apple.html','w');
fprintf(fid,'<html><body>\n');
for i=1:length(result_cat_0)
	picname = result_cat_0{i};
...
result_cat_1 = {};
for k=1:100
    result_cat_1{k} = test_objs{I(k)};
end
% create a webpage showing the results
fid = fopen('cars.html','w');
fprintf(fid,'<html><body>\n');
for i=1:length(result_cat_1)
	picname = result_cat_1{i};
	fprintf(fid,['<img src="Img_Spanish/', picname,  '">\n']);
	if mod(i,3)==0  
		fprintf(fid,'<br>');
	end
...
result_cat_2 = {};
for k=1:100
    result_cat_2{k} = test_objs{I(k)};
end
% create a webpage showing the results
fid = fopen('roses.html','w');
fprintf(fid,'<html><body>\n');
for i=1:length(result_cat_2)
	picname = result_cat_2{i};
	fprintf(fid,['<img src="Img_Spanish/', picname,  '">\n']);
	if mod(i,3)==0  
		fprintf(fid,'<br>');
	end
...
result_cat_3 = {};
for k=1:100
    result_cat_3{k} = test_objs{I(k)};
end
% create a webpage showing the results
fid = fopen('tiger.html','w');
fprintf(fid,'<html><body>\n');
for i=1:length(result_cat_3)
	picname = result_cat_3{i};
	fprintf(fid,['<img src="Img_Spanish/', picname,  '">\n']);
	if mod(i,3)==0  
		fprintf(fid,'<br>');
	end
